    ...
    <div class="star-rating">
        <div v-for="(star, index) in stars" :key="index" class="star-container">
            <svg class="star-svg" :style="[
                ...
                <polygon :points="getStarPoints" style="fill-rule:nonzero;"></polygon>
                <defs>
                    <linearGradient :id="`gradient${star.raw}`">
                        <stop id="stop1" :offset="star.percent" stop-opacity="1" :stop-color="getFullFillColor(star)">
                        </stop>
                        <stop id="stop2" :offset="star.percent" stop-opacity="0" :stop-color="getFullFillColor(star)">
                        </stop>
                        <stop id="stop3" :offset="star.percent" stop-opacity="1" :stop-color="style.emptyStarColor">
                        </stop>
...
<script>
    export default {
        ...
        methods: {
            ...
            setStars() {
                let fullStarsCounter = Math.floor(this.ratings);
                for (let i = 0; i < this.stars.length; i++) {
                    if (fullStarsCounter !== 0) {
                        ...
                        let surplus = Math.round((this.ratings % 1) * 10) / 10; // Support just one decimal
