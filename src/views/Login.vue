      ...
      <div class="signin-form-block">
        ...
        <b-form class="signin-form">
          ...
          <b-form-group>
            ...
            <div
              v-if="submitted && !$v.login.password.minLength"
              class="invalid-feedback"
...
export default {
  ...
  methods: {
    handleSubmit(e) {
      ...
      axios
        .post(process.env.VUE_APP_API_ENDPOINT + "login", this.login)
        .then(response => {
  ...
  created() {
    ...
    axios
      .get(process.env.VUE_APP_API_ENDPOINT + "/connect")
      .then(response => {
        if (response.data.data.language) {
          //convert language object to array
          let listOfObjects = Object.keys(response.data.data.language).map(
            key => {
